...
contract("League", (accounts) => {
    ...
    it("test createLeague - user should not be able to create two leagues", async () => {
        let leagueName = "Test League";
        await contractInstance.createLeague(tournamentID, leagueName, {from: alice});

        await utils.shouldThrow(contractInstance.createLeague(tournamentID + 1, "Not the Same", {from: alice}));
